...
def load_expt(expt, vari):
    ...
    cube_list = iris.load(monthly_file_location(expt, vari), lat_bounds(-7, 8) & \
                            lon_bounds(30, 45) & \
                            pressure_level(expt, 85000) & year_bounds(1980, 2014))
...
if processor_calculations:
    ...
    for expt in green_list:
      ...
      iris.io.save(mflx_clim, starternc+expt+'_mflx_clim'+nc_file)
      pickle.dump(ua_clim, open(starterp+expt+'_ua_clim'+p_file, "wb" ))
      iris.io.save(ua_clim, starternc+expt+'_ua_clim'+nc_file)
      pickle.dump(va_clim, open(starterp+expt+'_va_clim'+p_file, "wb" ))
      iris.io.save(va_clim, starternc+expt+'_va_clim'+nc_file)
