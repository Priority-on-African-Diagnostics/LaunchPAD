...
def load_expt_t(expt):
    ...
    cube_list = iris.load(monthly_file_location(expt, vari), year_bounds(1990, 1991))         
...
def load_expt_wd(expt):
    ...
    result250 = cube.extract(pressure_level(expt,25000))
    result1000 = cube.extract(pressure_level(expt,100000))
    result850 = cube.extract(pressure_level(expt,85000))
...
def calc_mslp(l):
  ...
  if(l == 0) or (tc_id[l-1] == 0):
    ...
    for j in range(int_lat_lower,int_lat_upper): #this assumes interval =1
...
if pre_processor_experiments:
    ...
    for expt in green_list:   
        t250,t500 = load_expt_t(expt, 'ta')
        pickle.dump(t250, open(starterp+expt+'_'+'_t250_'+p_file, "wb" ))
        pickle.dump(t500, open(starterp+expt+'_'+'_t500_'+p_file, "wb" ))

        u250,u850,u1000 = load_expt_wd(expt, 'ua')
        v250,v850,v1000 = load_expt_wd(expt, 'va')
        ws250 = calc_windspeed(u250, v250)
        ws850 = calc_windspeed(u850, v850)
        ws1000 = calc_windspeed(u1000, v1000)
        pickle.dump(ws250, open(starterp+expt+'_'+vari+'_ws250_'+p_file, "wb" ))
        pickle.dump(ws850, open(starterp+expt+'_'+vari+'_ws850_'+p_file, "wb" ))
        pickle.dump(ws1000, open(starterp+expt+'_'+vari+'_ws1000_'+p_file, "wb" ))
...
if processor_calculations:
    ...
    for expt in green_list:
        ws250 = unpickle_cubes(starterp+expt+'_'+vari+'_ws250_'+p_file)
        threshold = ws_thresh(math.abs(ws250.coord('longitude').point[2]-ws250.coord('longitude').point[1])*111)
        nl=ws250.coord('time').point
        for x in range(0,nl):
            calc_mslp(x)
            calc_windregion(x)
            calc_maxwind(x)
            calc_vort(x)
            calc_warm_core(x)
            if(max_wspd[x] >= wspd_threshold): #check this nesting
                if(vort_min[x] <= -0.000035):
                    if(wc_temp[x] >= (avg_temp_500_250[x] + 1)):
                        if(avg_wspd_250[x] < avg_wspd_850[x]): 
                            tc_id[x]=1
                        else:
                            tc_id[x]=0
