...
def load_expt(expt, vari):
    ...
    print(monthly_file_location(expt, vari))
    return cube
...
def conv_volumetric(expt,var2):
    ...
    # m layer depth -> mm
    if expt in mod_list:  
      vm = (var2[:,0,:,:]/(1000.0*var2.coord('depth').points[0]))
    else: 
      vm = var2
    return vm
...
def plot_TCI(expt,seasons):

    plt.figure(figsize=(7, 7))
    #CHANGE
    clevs = np.arange(-0.05,0.06,.01)
    #CHANGE
