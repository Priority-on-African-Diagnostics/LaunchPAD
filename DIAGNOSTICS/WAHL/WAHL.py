...
def plot_WAHL(expt, vari):
     ...
     fig.suptitle(vari_line+' @ 0600 for '+expt+' 1983-2012 (m) ')
     ...
     for counter, Cube1 in enumerate(Cube0.slices(['latitude','longitude'])):
	     ...
	     #subtract from LLAT
             ...
             if vari == 'pr':
                 Cube1.data = np.ma.masked_where(Cube1.data <= 5, Cube1.data)
             if vari == 'ts':
                 Cube1.data = np.ma.masked_where(Cube1.data <= 0, Cube1.data)
